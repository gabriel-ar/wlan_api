...
namespace WlanApi {
    ...
    public class WlanClient {
        ...
        public class WlanInterface {
            ...
            /// <summary>
            /// Disconnects of the network.
            /// </summary>
            public void Disconnect() {
                var result = Wlan.WlanDisconnect(client.clientHandle, info.interfaceGuid, IntPtr.Zero);
                }
        ...
        public delegate void HostedNetworkEventHandler(Wlan.WlanNotificationData notif_data);
        public event HostedNetworkEventHandler HostedNetworkNotification;
        ...
        private void OnWlanNotification(ref Wlan.WlanNotificationData notifyData, IntPtr context) {
            ...
            switch(notifyData.notificationSource) {
                case Wlan.WlanNotificationSource.ACM:
                    switch((Wlan.WlanNotificationCodeAcm) notifyData.notificationCode) {
                        ...
                        case Wlan.WlanNotificationCodeAcm.ScanFail:
                                ...
                                int expectedSize;
                                try {
                                    expectedSize = Marshal.SizeOf(typeof(Wlan.WlanReasonCode));
                                    } catch(Exception) {
                                    expectedSize = 0;
                                    }
                                if(notifyData.dataSize >= expectedSize) {
                ...
                case Wlan.WlanNotificationSource.HNWK:
                    Debug.Print("HNWK");
                    switch((Wlan.HostedNetworkNotificationCode) notifyData.notificationCode) {
                        ...
                        case Wlan.HostedNetworkNotificationCode.RadioStateChange:
                                HostedNetworkNotification(notifyData);
                            break;
                        }
        ...
        #region HostedNetwork

        public Wlan.HostedNetworkReason HostedNetworkStart() {
            Wlan.HostedNetworkReason reason;
            Wlan.WlanHostedNetworkStartUsing(clientHandle, out reason, IntPtr.Zero);

            if(HostedNetworkStatus().HostedNetworkState != Wlan.HostedNetworkState.Active) {
                Wlan.WlanHostedNetworkForceStart(clientHandle, out reason, IntPtr.Zero);
                }   

            return reason;
            }

        public Wlan.HostedNetworkReason HostedNetworkStop() {
            Wlan.HostedNetworkReason reason;
            Wlan.WlanHostedNetworkStopUsing(clientHandle, out reason, IntPtr.Zero);

            if(HostedNetworkStatus().HostedNetworkState != Wlan.HostedNetworkState.Idle) {
                Wlan.WlanHostedNetworkForceStop(clientHandle, out reason, IntPtr.Zero);
                }
        ...
        public Wlan.WlanHostedNetworkStatus HostedNetworkStatus() {
            IntPtr Status;
            Wlan.WlanHostedNetworkQueryStatus(clientHandle, out Status, IntPtr.Zero);
            return (Wlan.WlanHostedNetworkStatus) Marshal.PtrToStructure(Status, typeof(Wlan.WlanHostedNetworkStatus));
            }
        ...
        public Wlan.WlanOpcodeValueType HostedNetworkQuerySecurity(out Wlan.WlanHostedNetworkSecuritySettings secutity) {

            uint data_size;
            IntPtr data_ptr;
            Wlan.WlanOpcodeValueType value_type;

            Wlan.WlanHostedNetworkQueryProperty(clientHandle, Wlan.HostedNetworkOpcode.SecuritySettings, out data_size, out data_ptr, out value_type, IntPtr.Zero);

            secutity = (Wlan.WlanHostedNetworkSecuritySettings) Marshal.PtrToStructure(data_ptr, typeof(Wlan.WlanHostedNetworkSecuritySettings));
        ...
        public static Wlan.Dot11Ssid CreateDot11Ssid(String SSID) {
            byte[] bt_ssid = new byte[32];
            Encoding.UTF8.GetBytes(SSID, 0, SSID.Length, bt_ssid, 0);
            return new Wlan.Dot11Ssid() { SSID = bt_ssid, SSIDLength = (uint) SSID.Length };
            }
